...
```{r}
# Danh gia du lieu
cor(vin2[,2:6]) #Correlation matrix
corrplot(cor(vin2[,2:6]),method = "number", order = "hclust")
```

```{r}
# bieu do giua ngay va giabinhquan
ggplot(data = vin2, mapping = aes(date, adj)) + geom_point() +
...
```{r}
knn_pred_amzn <- predict(knn_amzn, newdata = test_amzn)
